...
*/
	...
	if(options.network.users && options.network.users.length > 0){
		...
		async.each(arr, function(i, a_cb) {
			if(options.network.users[i]){
				logger.log(options.network.users);										//make sure we still have a enrollId for this network
				var maxRetry = 2;
...
*/
	...
	*/var unzip_dest = path.join(tempDirectory,  '/unzip');								//	=./temp/unzip
	...
	/*function cb_ready(){
		try{fs.mkdirSync(tesmpDirectory);logger.log(tempDirectory);}
		catch(e){ }
	...
	var filename = path.join('/home/akarsh/Documents', '/BookApplication');
	fs.readdir(filename, cb_got_names);
			...
			for(var i in obj){
				if(obj[i].indexOf('.go') >= 0){											//look for GoLang files
					if(!found_invoke || !found_query){
						foundGo = true;
						var file = filename//fs.readFileSync(path.join(unzip_cc_dest, obj[i]), 'utf8');

						// Step 2a.
						ibc.chaincode.details.version = 'github.com/hyperledger/fabric/core/chaincode/shim'//find_shim(file);
	...
	function parse_for_invoke(name, str){
		if(str == null) logger.error('! [ibc-js] fs readfile Error');
			...
			// Step 2a.
			var go_func_regex = /func\s+\(\w+\s+\*BookChaincode\)\s+(\w+)/g;			//find chaincode's go lang functions
			var result;
...
function deploy(func, args, deploy_options, enrollId, cb){
	...
	body = 	{
		...
		params: {
			type: 1,
			chaincodeID:{
				path: '/home/akarsh/Documents/BookApplication'
			},
			ctorMsg: {
				function: func,
				args: args
